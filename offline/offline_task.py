...
def generateMenuTreeJSON():
    ...
    o = generateMenu(loadFromDBFlag, allNodesStandardState, filterFlag, filterText)
    return o
...
def Histo(path=""):
    ...
    while i < len(histosContained):
        ...
        if i != 0 and (i % 2) == 0:
            rows += render_template("histoDB/histoRow.html", COLUMNS = columns)
            columns = ""
        #end if	
        ...
        #save into template
        columns += render_template("histoDB/histoCell.html", 
    ...
    if columns != "":
        ...
        page = render_template("histoDB/Histo.html", PATH = path, 
